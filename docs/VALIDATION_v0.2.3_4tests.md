# xaxiu-swarm v0.2.3 — Test-Driven Patch Batch (4 Workloads)
...
### Test 3 — Real V_CONV4f wave end-to-end via xaxiu-swarm primitives

**Setup:** Smallest-possible content (1-line defensive guard for V_CONV4e DI-DeepSeek "Finding 6a" undefined `baseConn.ct`). Full wave protocol via xaxiu-swarm:
- SHIP: `xaxiu-swarm dispatch --backend kimi`
- Cycle 1: `xaxiu-swarm swarm --backends kimi,deepseek --context-file <V-file>`
- AG#1: `xaxiu-swarm dispatch --backend deepseek --context-file <reports> --deliverable <path>`

**v0.2.2 result:** ✅ V_CONV4f shipped CLEAN end-to-end via xaxiu-swarm primitives.
...
**Findings:** No new bugs. xaxiu-swarm v0.2.2 is validated as drop-in replacement for `kimi_dispatch.py` + `ask_kimi.py` at full-wave-protocol scope.

### Test 4 — Long-running single Kimi dispatch

**Setup:** Single `xaxiu-swarm dispatch --backend kimi --timeout 5400 --max-iterations 60 --audit-max-len 200000`. Task: 5-pass exhaustive review of V_CONV4e (FIX markers, setConveyorScenario callsites, useCallback/useMemo deps, TODO/FIXME triage, code smells).
...
## Cumulative xaxiu-swarm validation surface (v0.1.0 → v0.2.3)
...
- Wave protocol scope tested: V_CONV4d (4-engine cohort), V_CONV4e (4-engine mixed cohort), V_CONV4f (full wave end-to-end via xaxiu-swarm primitives only)
