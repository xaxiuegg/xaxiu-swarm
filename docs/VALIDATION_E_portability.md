...
**agent-swarm version:** v0.2.1

## Method

1. `mkdir D:/Projects/agent-swarm-portability-test`
2. `cp -r infrastructure/kimi-swarm ./agent-swarm` (verbatim copy of the package dir from warehouse)
3. Removed stale `agent_swarm.egg-info/` and `.pytest_cache/` from copy
4. `python -m venv venv` (isolated Python environment, separate from warehouse's editable install)
5. `pip install -e ./agent-swarm`
...
```
Successfully installed agent-swarm-0.2.1 annotated-types-0.7.0 anyio-4.13.0
...
```
$ python -c "import agent_swarm; print(agent_swarm.__version__)"
0.2.1

$ python -m agent_swarm.cli backends
claude
deepseek
kimi
qwen
...
```
$ python -m agent_swarm.cli dispatch "Say exactly: 'portability test ok'." \
...
```
$ time python -m agent_swarm.cli dispatch "Say exactly: 'portability kimi ok'." \
...
**🟢 E PASS.** agent-swarm v0.2.1 is genuinely portable. No warehouse-specific imports, paths, or assumptions. Drop the directory into any project, run `pip install -e .`, set the relevant env vars (DEEPSEEK_API_KEY, ANTHROPIC_API_KEY, etc.), and it works.

For a future dedicated repo: `git init D:/Projects/agent-swarm/`, copy the package contents, push to a new GitHub repo. No vendor-fork burden.
...
## Cumulative agent-swarm validation surface (v0.1.0 → v0.2.1)
...
- Git commits in agent-swarm series: a29da3f → f0e3929 → ed15a12 → 88025a9 → 2fe5ac6 → b1d57f2 → d64803e → 6df4dc7
